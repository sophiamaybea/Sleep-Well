...
- **Social Features:** Includes Tending (follow system with loading/error states), Resonance (five reaction types: glow, pressed_flower, dewdrop, firefly, roots), Marginalia (annotation-style comments), Notifications ("Wind Chimes" panel in navigation with unread badge), quiet read whispers (anonymized reader details like "a poet lingered here yesterday"), ambient garden presence ("3 writers in the garden right now" with monthly summary), Gallery Comments ("Reflections" section on published pieces), Bookmarks (save pieces from Gallery reading view), and Share button (Web Share API + clipboard fallback).
...
- **Content Management:** Implements rich text editing via TipTap with adjustable font size (slider 14-24px) and 5 font options (Serif, Display, Typewriter, Mono, Sans), piece organization with custom tags, pinning/unpinning, and archiving. Stage tooltips on readiness buttons. Writing analytics dashboard (total pieces, word count, avg length, reading time).
- **Daily Writing Prompts:** Auto-generated daily prompts from a curated list of 30 prompts across 5 categories, displayed in Garden dashboard with "Write from this prompt" action.
- **Portfolio:** Pinned pieces displayed as "Featured Works" on public garden pages with amber-themed styling.
- **Craft Guides:** Five categories of writing tips (Poetry, Fiction, Essay, Revision, Publishing) in the Field Guide page.
