    ...
    return (
      <main className="min-h-screen bg-[#f7f4ee] text-[#1f1d18] flex items-center justify-center px-6">
        <div className="max-w-md text-center space-y-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-black/70">Restricted Space</p>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="rounded-2xl border border-black/8 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            <div>
              <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-black/70">Editor Studio</p>
        ...
        <section className="grid xl:grid-cols-[1.05fr,1fr] gap-4">
          <div className="rounded-2xl border border-black/8 bg-white p-4 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-3">
              <div className="relative flex-1">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-black/60" />
                <input
                  type="text"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search by title, text, or genre"
                  className="w-full rounded-xl border border-black/10 bg-[#fcfbf8] py-2 pl-9 pr-3 text-sm text-black/80 placeholder:text-black/65 focus:outline-none focus:border-[#29493d]/35"
                />
              </div>
                ...
                {["all", "triage", "development", "ready"].map((filter) => (
                  <button
                    ...
                    className={`rounded-lg px-2.5 py-1 font-mono text-[8px] uppercase tracking-widest transition-colors ${
                      bucket === filter ? "bg-white text-black/80 shadow-sm" : "text-black/70 hover:text-black/90"
                    }`}
                  >
            ...
            >
              {isFetching && <p className="font-mono text-[10px] uppercase tracking-widest text-black/70">Loading queue...</p>}
              {!isFetching && enriched.length === 0 && (
                <div className="rounded-xl border border-dashed border-black/12 p-6 text-center">
                  <p className="text-sm text-black/75">No pieces match this filter.</p>
                </div>
              )}
              {enriched.map((item) => {
          ...
          <div className="rounded-2xl border border-black/8 bg-white p-4 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
            <p className="mb-3 font-mono text-[9px] uppercase tracking-[0.25em] text-black/70">Active Edit Pass</p>
            {!selected ? (
              <p className="text-sm text-black/75">Select a piece from the queue.</p>
            ) : (
              <div className="space-y-4">
                <div>
                  <h2 className="text-2xl font-semibold tracking-tight text-black/90">{selected.writing.title || "Untitled"}</h2>
                  <p className="mt-1 font-mono text-[9px] uppercase tracking-widest text-black/70">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                  <p className="font-mono text-[8px] uppercase tracking-widest text-black/70">Checklist {doneCount}/4 complete</p>
                  <a
