        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111] border border-white/10 rounded-xl p-6 w-full max-w-md">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 font-mono text-sm text-white placeholder-white/20 mb-3 outline-none focus:border-emerald-700/50"
            />
            <textarea
              ...
              rows={3}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 font-mono text-sm text-white placeholder-white/20 mb-3 outline-none focus:border-emerald-700/50 resize-none"
            />
            <label className="flex items-center gap-2 mb-4 cursor-pointer">
              ...
              />
              <span className="font-mono text-xs text-white/50">Make this bed visible in the Garden</span>
            </label>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-white/30 font-mono text-xs hover:text-white/60 transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        {isLoading ? (
          <div className="text-center py-16">
            <p className="font-mono text-xs text-white/20">Tending...</p>
          </div>
        ) : collections.length === 0 ? (
          <div className="text-center py-16">
            <BookOpen size={32} className="mx-auto mb-4 text-white/10" />
            <h2 className="font-serif text-lg text-white/40 mb-2">No beds yet.</h2>
          ...
          <div className="space-y-3">
            {collections.map((c) => (
              <Link key={c.id} href={`/garden/collections/${c.id}`}>
                <div className="group border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] rounded-xl p-5 cursor-pointer transition-all hover:shadow-lg hover:-translate-y-0.5 duration-200">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {c.description && (
                        <p className="font-mono text-xs text-white/30 line-clamp-1">{c.description}</p>
                      )}
                    ...
                    {c.shareSlug && (
                      <button
                        ...
                        className="ml-3 p-2 text-white/20 hover:text-white/60 transition-colors opacity-0 group-hover:opacity-100"
